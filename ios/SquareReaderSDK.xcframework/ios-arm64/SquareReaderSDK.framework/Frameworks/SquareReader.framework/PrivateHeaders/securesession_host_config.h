...
//
// Securesession lets us assert control of what its crypto API's to use are through the
// HOST_CRYPTO_API macro. This is done to assure effective inlining from Arxan still takes place.
// Otherwise it will use function pointers which will prevent inlining of any crypto calls
//
#define SECURESESSION_HOST_CRYPTO_API(_ctx, _function_name) SECURESESSION_HOST_WRAPPER_##_function_name
