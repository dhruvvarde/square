...
@protocol CRPaymentLoggingDelegate <NSObject>

/**
 Message sent when there is a PIN entry event occurring on the PIN Entry Device (PED).
 The `eventText` describes the PIN event and may be used for logging.
 */
- (void)paymentPINEventOnReader:(NSString *)eventText;
...
@property (nonatomic, weak) id<CRPaymentLoggingDelegate> loggingDelegate;
